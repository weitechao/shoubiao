...
//	public class LocationUdService implements IService {
	...
	protected String process2(SocketLoginDto socketLoginDto, String jsonInfo, Channel channel) {
		...
		if("A".equals(locationis)){
			...
			locationService.insertUdInfo(imei,1,lat,lon,status,time,locationStyle);	
			...
			String url="http://apilocate.amap.com/position?key="+Utils.SSRH_LOCATION_KEY+"&output=json&accesstype=0&imsi="+imei+"&cdma=0&tel=13537596170&network=GSM&"+sbb.toString()+sb.toString();
			logger.info(url);
			...
			if (responseJsonString != null) {
				...
				if ("1".equals(status)) {
					...
					String	lat = arr[1];
					String	lon = arr[0];
					locationService.insertUdInfo(imei,2,lat,lon,status,time,locationStyle);	
					}
				}
			}
			}else{
				...
				String url="http://apilocate.amap.com/position?key="+Utils.SSRH_LOCATION_KEY+"&output=json&accesstype=1&mmac="+mmac+"&macs="+macs;
				logger.info(url);
				String responseJsonString = HttpClientGet.urlReturnParams(url);
				if (responseJsonString != null) {
					JSONObject responseJsonObject = (JSONObject) JSON.parse(responseJsonString);
					...
					if ("1".equals(status)) {
						...
						String	lat = arr[1];
						String	lon = arr[0];
						locationService.insertUdInfo(imei,3,lat,lon,status,time,locationStyle);	
						}
					}
				}
			}
			...
			if(wifiCount>0){
				...
				if (responseJsonString != null) {
					...
					if ("1".equals(status)) {
						...
						String	lat = arr[1];
						String	lon = arr[0];
						locationService.insertUdInfo(imei,3,lat,lon,status,time);	
						}
					}
				}
